...
var gokwikEnabled=!!(window.theme&&window.theme.settings&&window.theme.settings.use_gokwik_checkout);
if(!gokwikEnabled)return false;
...
if(window.theme&&window.theme.settings&&window.theme.settings.use_gokwik_checkout){
...
h+='<form action="'+resolveCheckoutUrl()+'" method="post" novalidate class="cart-drawer__checkout-form shopifycheckoutbtn" style="display:none"><button type="submit" name="checkout" class="btn btn--primary btn--full">Checkout</button></form>';
}else{
h+='<form action="'+resolveCheckoutUrl()+'" method="post" novalidate class="cart-drawer__checkout-form shopifycheckoutbtn"><button type="submit" name="checkout" class="btn btn--primary btn--full">Checkout</button></form>';
}
...
var targetIndex=-1;
/* Strategy 1: full variant match has featured_image.position */
if(match&&match.featured_image&&match.featured_image.position){
targetIndex=match.featured_image.position-1;
}
/* Strategy 2: no full match — find first variant for the clicked color using its featured_image.position */
...
/* Strategy 3: positional grouping (no featured_image data at all) */
